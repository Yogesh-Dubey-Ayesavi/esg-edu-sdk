 ...
 * @param {string} fileId - The id of the file for which you are fetching content
 * @returns {Promise<FileContent>} - A Promise that resolves to an array of FileContent instances.
 */
export default async function getFileContent(dir: string, fileId:string): Promise<FileContent> {
    try {
